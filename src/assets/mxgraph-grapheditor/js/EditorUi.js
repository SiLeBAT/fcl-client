...
EditorUi.prototype.chooseAndOpenLocalFile = function() {
    const currentFile = this.currentFile;
    const fnChooseAndOpen = () => {
        if (
            ...
            navigator.appVersion.indexOf('Windows NT 6.1') < 0)
                ...
                mxEvent.addListener(input, 'change', mxUtils.bind(this, function()
                {
                    if (input.files != null)
                    {
                        this.openLocalFile(input.files[0]);
                    }
                }));
    ...
    if (
        (currentFile == null && !this.isDiagramEmpty()) ||
        (currentFile != null && currentFile.isModified())) {
		this.confirm(mxResources.get('allChangesLost'), fnChooseAndOpen, null,
...
EditorUi.prototype.openLocalFile = function(file) {
...
EditorUi.prototype.saveFile = function(forceDialog)
{
	...
	else
	{
		...
		this.showDialog(dlg.container, 360, 160, true, true);
		dlg.init();
	}
};
		...
		try
		{
			...
			else
			{
                ...
                if (window.navigator.msSaveOrOpenBlob != null) {
                    ...
                    // openSaveDialog(url: string, fileName: string) {
