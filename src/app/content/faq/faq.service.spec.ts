...
describe('FaqService', () => {
    ...
    it('should load faq data correctly', async () => {
        return faqService.getFaq().toPromise()
            .then(faq => {
                expect(faq).toBeTruthy();
                expect(faq.sections.length).toBeGreaterThan(0);
                const emptySectionTitleExist = faq.sections.some(s => (s.title || '').length === 0);
                expect(emptySectionTitleExist).toBe(false);

                const emptyUrlFragmentExist = faq.sections.some(s => (s.urlFragment || '').length === 0);
                expect(emptyUrlFragmentExist).toBe(false);

                const sectionWoQuestionExist = faq.sections.some(s => s.entries.length === 0);
                expect(sectionWoQuestionExist).toBe(false);

                const emptyQuestionsExist = faq.sections.some(s => s.entries.some(e => (e.question || '').length === 0));
                expect(emptyQuestionsExist).toBe(false);

                const emptyAnswersExist = faq.sections.some(s => s.entries.some(e => (e.answer || '').length === 0));
