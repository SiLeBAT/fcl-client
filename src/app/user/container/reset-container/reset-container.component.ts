...
export class ResetContainerComponent {
    ...
    reset(password: NewPasswordRequestDTO) {
        ...
        this.userService.resetPassword(password, token).subscribe(
            (resetResponse: PasswordResetResponseDTO) => {
            ...
            (err) => {
                ...
                if (err instanceof InvalidServerInputHttpErrorResponse) {
                    ...
                    this.alertService.error(
                        "The password could not be updated. The password is not compliant with the password rules.",
                    );
                } else {
                    this.alertService
                        .error(`Error during password reset, the token is not valid.
