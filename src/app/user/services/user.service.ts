...
export class UserService {
  ...
  register(
    credentials: RegistrationDetailsDTO
  ): Observable<RegistrationRequestResponseDTO> {
  ...
  recoverPassword(
    email: ResetRequestDTO
  ): Observable<PasswordResetRequestResponseDTO> {
    return this.dataService.put<
      PasswordResetRequestResponseDTO,
      ResetRequestDTO
    >(this.URL.recovery, email);
  }

  resetPassword(
    newPw: NewPasswordRequestDTO,
    token: string
  ): Observable<PasswordResetResponseDTO> {
    return this.dataService.patch<
      PasswordResetResponseDTO,
      NewPasswordRequestDTO
    >([this.URL.reset, token].join('/'), newPw);
  }

  activateAccount(token: string): Observable<ActivationResponseDTO> {
    return this.dataService.patch<ActivationResponseDTO, string | null>(
      [this.URL.activate, token].join('/'),
      null
    );
  }
  ...
  adminActivateAccount(adminToken: string): Observable<ActivationResponseDTO> {
    return this.dataService.patch<ActivationResponseDTO, string | null>(
      [this.URL.adminactivate, adminToken].join('/'),
      null
    );
  }

  login(credentials: LoginCredentials): Observable<TokenizedUserDTO> {
    return this.dataService.post<TokenizedUserDTO, LoginCredentials>(
      this.URL.login,
      credentials
    );
  }
  ...
  updateGDPRAgreement(
    user: GdprConfirmationRequestDTO
  ): Observable<TokenizedUserDTO> {
    return this.dataService.put<TokenizedUserDTO, GdprConfirmationRequestDTO>(
      this.URL.gdpragreement,
      user
    );
  }
  ...
  confirmNewsletterSubscription(
    token: string
  ): Observable<NewsConfirmationResponseDTO> {
    return this.dataService.patch<NewsConfirmationResponseDTO, string | null>(
      [this.URL.newsconfirmation, token].join('/'),
      null
    );
  }
