...
export class LPModel {
  ...
  addConstraint(
    ...
    for (const varName of Object.getOwnPropertyNames(term)) {
      if (
        !Object.prototype.hasOwnProperty.call(this.model[LPModel.VARS], varName)
      ) {
  ...
  private getDifferences(
    ...
    return {
      absDiff: absDiff,
      relDiff:
        absDiff / Math.max(Math.abs(lessOrEqValue), Math.abs(greaterOrEqValue)),
    };
  }
  ...
  private termToString(
    ...
    for (const variableId of variableIds) {
      ...
      const absSubTerm =
        (roundedAbsCoeff !== 1 ? roundedAbsCoeff + ' ' : '') + variableId;
