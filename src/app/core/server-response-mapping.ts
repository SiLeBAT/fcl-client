    ...
    return (
        ...
        error.error.errors.every(
            (valdationError: ValidationError) => (
                ...
                typeof valdationError.code === 'string' && valdationError.code !== '' &&
                typeof valdationError.message === 'string' && valdationError.message !== ''
            )
        )
    );
}

export function fromErrorExtractInvalidDataErrorDTO(error: unknown): InvalidDataErrorDTO {
