...
export class DashboardContainerComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.store.dispatch(
      new mainPageActions.DashboardActivated({isActivated: true})
    );
  }

  ngOnInit() {
    const currentUser$: Observable<TokenizedUser> = this.store
    ...
    currentUser$
      .pipe(
        ...
        exhaustMap((currentUser: TokenizedUser) =>
          this.userService.openGDPRDialog().pipe(
            takeWhile(() => this.componentActive),
                ...
                return this.userService
                  ...
                  .pipe(
                    ...
                    map((mapResult: TokenizedUserDTO) => {
                      ...
                      this.store.dispatch(
                        new userActions.UpdateUserSOA({currentUser: mapResult})
                      );
                    }),
                    take(1)
                  );
  ...
  ngOnDestroy() {
    ...
    this.store.dispatch(
      new mainPageActions.DashboardActivated({isActivated: false})
    );
  }
}
