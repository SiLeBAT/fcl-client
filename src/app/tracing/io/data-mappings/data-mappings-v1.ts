...
export const DONT_APPLY_VALUES_FOR_EXT_STATION_COLS: ImmutableList<string> =
  ImmutableList([
...
export const DONT_APPLY_VALUES_FOR_EXT_DELIVERY_COLS: ImmutableList<string> =
  ImmutableList([
...
export const DEFAULT_STATION_PROP_INT_TO_EXT_MAP: ImmutableMap<string, string> =
  ImmutableMap({
...
export const DEFAULT_DEL2DEL_PROP_INT_TO_EXT_MAP: ImmutableMap<string, string> =
  ImmutableMap({
...
export const DENOVO_DELIVERY_PROP_INT_TO_EXT_MAP: ImmutableMap<string, string> =
  ImmutableMap(
    Object.assign(DEFAULT_DELIVERY_PROP_INT_TO_EXT_MAP.toObject(), {
...
function getPropMap(
  ...
  altMapList.forEach((propSet: AltPropMap | undefined) => {
    ...
    if (
      intProps.some(
        intProp => availableExtPropsSet[propMap[intProp]] === undefined
      )
    ) {
  ...
  unmappedExtProps.forEach(extProp => {
    if (
      ...
      propMap[
        'external' + extProp.slice(0, 1).toUpperCase() + extProp.slice(1)
      ] = extProp;
    }
  });
  ...
  if (highlightingConditions) {
    ...
    for (const hCon of highlightingConditions) {
      ...
      if (hCon.logicalConditions) {
        getLogicalConditionsProps(hCon.logicalConditions).forEach(p =>
          propSet.add(p)
        );
      }
...
function getLogicalConditionsProps(
  logicalConditions: ExtLogicalCondition[][]
): string[] {
  const propSet = new Set<string>();
...
export function getStationPropMap(jsonData: JsonData): PropMap {
  ...
  if (
    ...
    if (jsonData.settings.view.node.highlightConditions) {
      referencedProps = getReferencedProps(
...
export const GROUPTYPE_EXT_TO_INT_MAP: ImmutableMap<string, GroupType> =
  ImmutableMap({
...
export function NODE_SIZE_EXT_TO_INT_FUN(ext: string | number): number {
  ...
  if (isNaN(extV)) {
    ...
    return IntDataConstants.NODE_SIZES.toArray().reduce(
      (prevV, curV) =>
        Math.abs(prevV - extV) > Math.abs(curV - extV) ? curV : prevV,
...
export function EDGE_WIDTH_EXT_TO_INT_FUN(extValue: number): number {
  if (isNaN(extValue)) {
    ...
    return IntDataConstants.EDGE_WIDTHS.toArray().reduce(
      (prevV, curV) =>
        Math.abs(prevV - extValue) > Math.abs(curV - extValue) ? curV : prevV,
...
export const OPERATION_TYPE_EXT_TO_INT_MAP: ImmutableMap<
  string,
  OperationType
> = ImmutableMap({
...
export const VALUE_TYPE_EXT_TO_INT_MAP: ImmutableMap<string, ValueType> =
  ImmutableMap({
    VALUE: ValueType.VALUE,
    LOG_VALUE: ValueType.LOG_VALUE,
  });

export const NODE_SHAPE_TYPE_EXT_TO_INT_MAP: ImmutableMap<
  string,
  NodeShapeType
> = ImmutableMap({
...
export class PropMapper {
  ...
  applyValuesFromTableRow(
    fromRow: DataRow,
    toObj: Partial<Connection | StationStoreData | DeliveryStoreData>
  ): void {
