...
function createDeliveryRelationTable(fclData: FclData): DataTable {
  const columnSourceId =
    DataMapper.DELIVERY_TO_DELIVERY_PROP_INT_TO_EXT_MAP_V_FROM_TO.get(
      'source'
    ).columnId;
  ...
  return {
    ...
    data: concat(
      ...fclData.fclElements.stations.map(station =>
...
function collectDeliveryProperties(
  fclData: FclData,
  ...
  fclData.fclElements.deliveries.forEach(delivery =>
    delivery.properties.forEach(stringProp =>
      propCollector.push(
        propMap[stringProp.name] || stringProp.name,
        ValueType.STRING
      )
    )
  );
...
function collectStationProperties(
  fclData: FclData,
  propMap: {[key: string]: string}
): ColumnProperty[] {
  const propCollector = new PropCollector();
  ...
  fclData.fclElements.stations.forEach(station =>
    station.properties.forEach(stringProp =>
      propCollector.push(
        propMap[stringProp.name] || stringProp.name,
        ValueType.STRING
      )
