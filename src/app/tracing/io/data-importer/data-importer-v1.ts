...
export class DataImporterV1 implements IDataImporter {
    ...
    private convertExternalData(data: JsonData, fclData: FclData) {
        const idToStationMap: Map<string, StationData> =
            this.applyExternalStations(data, fclData);
        const idToDeliveryMap: Map<string, DeliveryData> =
            this.applyExternalDeliveries(data, fclData, idToStationMap);
        const idToGroupMap: Map<string, GroupData> =
            this.applyExternalGroupData(data, fclData, idToStationMap);
        ...
        for (const stationRow of stationRows) {
            ...
            if (idToStationMap.has(intPartStation.id)) {
                throw new InputDataError(
                    "Duplicate station id:" + intPartStation.id,
                );
            }
    ...
    private applyExternalDeliveries(
        ...
        for (const deliveryRow of deliveryRows) {
            ...
            if (
                intPartDelivery.id === undefined ||
                intPartDelivery.id === null
            ) {
            ...
            if (idToDeliveryMap.has(intPartDelivery.id)) {
                throw new InputDataError(
                    "Duplicate delivery id:" + intPartDelivery.id,
                );
            }
            ...
            if (!idToStationMap.has(intPartDelivery.source)) {
                throw new InputDataError(
                    'Delivery source with id "' +
                        intPartDelivery.source +
                        '" is unkown.',
                );
            }
            ...
            if (!idToStationMap.has(intPartDelivery.target)) {
                throw new InputDataError(
                    'Delivery target with id "' +
                        intPartDelivery.target +
                        '" is unkown.',
                );
            }
            ...
            idToStationMap
                .get(intDelivery.source)!
                .outgoing.push(intDelivery.id);
            idToStationMap
                .get(intDelivery.target)!
                .incoming.push(intDelivery.id);
    ...
    private applyExternalDeliveryToDelivery(
        ...
        for (const del2DelRow of del2DelRows) {
            ...
            if (
                ...
                throw new InputDataError(
                    "Missing delivery to delivery source.",
                );
            }

            if (!idToDeliveryMap.has(partConnection.source)) {
                throw new InputDataError(
                    'Unkown delivery to delivery source "' +
                        partConnection.source +
                        '".',
                );
            }
            ...
            if (
                ...
                throw new InputDataError(
                    "Missing delivery to delivery target.",
                );
            }

            if (!idToDeliveryMap.has(partConnection.target)) {
                throw new InputDataError(
                    'Unkown delivery to delivery target "' +
                        partConnection.target +
                        '".',
                );
            }
    ...
    private applyExternalGroupData(
        ...
        const stationIds: Set<string> = new Set(
            Array.from(idToStationMap.keys()),
        );

        for (const extGroup of extGroups) {
            if (
                idToStationMap.has(extGroup.id) ||
                idToGroupMap.has(extGroup.id)
            ) {
                throw new InputDataError(
    ...
    private applyExternalTracingData(
        ...
        for (const element of deliveryTracings) {
            if (element.id === null) {
                throw new InputDataError(
                    "Delivery id is missing in tracing data.",
                );
            }
            ...
            if (tracSetIndex === undefined) {
                throw new InputDataError(
                    'Tracing-data-import: Delivery id "' +
                        element.id +
                        '" is unknown.',
                );
            }
    ...
    private importInvisibleElements(
        ...
        if (invDelIds !== undefined && invDelIds !== null) {
            const unknownIds = invDelIds.filter(
                (id) => !idToDeliveryMap.has(id),
            );
            if (unknownIds.length > 0) {
                throw new InputDataError(
                    'Unknown delivery id "' +
        ...
        if (extLogicalConditions) {
            intLogicalConditions = extLogicalConditions.map(
                (andConditionList: ExtLogicalCondition[]) =>
                    andConditionList.map(
                        (extCondition: ExtLogicalCondition) => {
    ...
    private mapValueType(extValueType: string): ValueType {
        if (DataMapper.VALUE_TYPE_EXT_TO_INT_MAP.has(extValueType)) {
            ...
            throw new Error(
                `Invalid ValueCondition.valueType: ${extValueType}`,
            );
        }
    }
