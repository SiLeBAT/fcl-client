...
import { TypeString2Type } from './xlsx-model';
import { TypeString } from './xlsx-model-v2';
...
export const COL_TYPES = {
    ...
    deliveries: {
        ...
        dateOut_month: 'month',
        dateOut_year: 'month',
        delAmountQuantity: 'nonneg:number',
        delAmountUnit: 'string',
        extId: 'string',
        indicator: 'string',
        itemNumber: 'nonneg:number',
        lotAmount_quantity: 'nonneg:number',
        ...
        subUnits: 'auto',
        target: 'string',
        unitAmount_quantity: 'nonneg:number',
...
} as const satisfies { [SR in SheetRef]: Record<ColumnRef<SR>, TypeString> };



type GetTableRowTypeFromDef<T extends TableRef> = typeof COL_TYPES[T];
type GetIgnoredFields<T extends Record<string, TypeString>> = {[key in keyof T]: key extends 'never' ? key : never }[keyof T];
type ImportedTableRow<T extends Record<string, TypeString>> = Omit<{ [key in keyof T]?: TypeString2Type<T[key]> }, GetIgnoredFields<T>>;
type ImportedDeliveryRow = ImportedTableRow<GetTableRowTypeFromDef<'deliveries'>>;
