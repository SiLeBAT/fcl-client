...
import { AdditionalColRefs, additionColumnGroups, ColRef2IndexMap, CustomRefs, EXT_JSON_NAMES, mandatoryColumnGroups, MANDATORY_COL_INDEXES, OTHER_PROP_REFS, Ref2FieldIndexMap, sheetNameMapping, SHEET_REFS } from './xlsx-all-in-one-import-const';
import { ColumnLabelMapping, ColumnLabelRef, ColumnRef, LabelGroupRef, LabelMapping, SheetRef, HeaderConf as RefHeaderConf, ColumnLabel, AdditionalStationRootColRef, AdditionalDeliveryRootColRef } from './xlsx-all-in-one-import-model';
import { Options, XlsxImporter } from './xlsx-importer-v0';
...
// function getOptionalColIndices<T extends SheetRef>(observedTableHeader: TableHeader, headerRefConf: DeepReadonly<RefHeaderConf<T>[]>): Partial<Record<AdditionalColRefs[T], number>> {
//     const ref2Index: Partial<Record<AdditionalColRefs[T], number>> = {};
//     headerRefConf.forEach(hC => {
//         const locHC = localizeHeaderConf()
//     } )
// }
...
function getOptionalColIndices<T extends SheetRef>(
    ...
    const headerRefConf = additionColumnGroups[sheetRef];
    const ref2Index: Partial<Record<AdditionalColRefs[T], number>> = {};
    headerRefConf.forEach((hC: DeepReadonly<RefHeaderConf<T>>) => {
        // const locHC = localizeHeaderConf()
    });
...
function importDeliveryRows(
    ...
    const headerConf = localizeHeaderConf('deliveries', labelMapping, additionColumnGroups.deliveries);
    const addColsIndices: Partial<Record<AdditionalDeliveryRootColRef, number>> = {};

    const colIndices: ColRef2IndexMap<'deliveries'> = {
