...
type TypeString = 'string' | 'number' | 'year' | 'month' | 'day';
...
interface ColumnsDefinition {
    [key: ColumnRef]: ColumnDefinition
}

interface RowHeader {
    rowIndex: number;
}
type TypeString2Type<T> = string extends T ? CellValue : T extends 'string' ? string : T extends 'number' ? number : unknown;
type GetTypeFromColumnDefinition<C extends { type?: TypeString }> = C extends { type: (infer T)} ? TypeString2Type<T> : never;
type GetMandatoryKeys<T extends ColumnsDefinition> =  { [key in keyof T]: T[key] extends { isMandatory: true } ? key : never }[keyof T];
