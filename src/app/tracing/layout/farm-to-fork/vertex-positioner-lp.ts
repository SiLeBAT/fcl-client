...
function shortenName(name: string, maxLen = 10): string {
  if (name === undefined) {
    return '';
...
class VertexPositionerLP {
  ...
  private addPositionConstraints(
    ...
    layers.forEach((layer, layerIndex) => {
      ...
      for (let iV = 1, nV: number = layer.length; iV < nV; iV++) {
  ...
  private addStraightLineConstraints(lpModel: LPModel, layers: Vertex[][]) {
    for (const layer of layers) {
      for (const vertex of layer) {
        ...
        const spanFactor =
          (nonVirtualSource.layerIndex - vertex.layerIndex) / layerSpan;
