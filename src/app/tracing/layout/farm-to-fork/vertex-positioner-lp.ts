...
import {
    getMinScaledVertexPairPosDistance,
...
export function positionVertices(
    layers: Vertex[][],
...
class VertexPositionerLP {
        ...
        for (const layer of layers) {
            ...
            if (layer.length > 0) {
                for (const vertex of layer) {
                    const solverValue: number = lpResult.vars.get(
                        buildPosVarName(vertex),
                    )!;
    ...
    private fitLayerScales(
        layers: Vertex[][],
        vertexDistance: number,
        maxLayerLength: number,
    ): void {
        const epsilon = maxLayerLength * 1e-6;
        let requiredLayerSpaces = layers.map((layer) =>
            getRequiredScaledLayerSpace(layer, vertexDistance),
        );
        ...
        while (maxRequiredSpace > maxLayerLength + epsilon) {
            ...
            requiredLayerSpaces = layers.map((layer) =>
                getRequiredScaledLayerSpace(layer, vertexDistance),
            );
    ...
    private fitLayerScale(
        ...
        while (requiredSpace > maxLayerLength + epsilon) {
            ...
            newLayerScale =
                (maxLayerLength - scaledComplementSpace) / unscaledSubSpace;
    ...
    private getScaledSpacesForLayer(
        layer: Vertex[],
        vertexDistance: number,
    ): ScaledSpace[] {
        ...
        layer.forEach((vertex, vertexIndex) => {
            ...
            if (vertexIndex > 0) {
                outerSpace += getMinVertexPairSpecificDistance(
                    layer[vertexIndex - 1],
                    vertex,
                    vertexDistance,
                );
            ...
            if (outerSpace > 0) {
                scaleToScaledSpaceMap[vertex.layerScale] =
                    (scaleToScaledSpaceMap[vertex.layerScale] || 0) +
                    outerSpace * vertex.layerScale;
            }
            if (innerSpace > 0) {
                scaleToScaledSpaceMap[vertex.innerScale] =
                    (scaleToScaledSpaceMap[vertex.innerScale] || 0) +
                    innerSpace * vertex.innerScale;
            }
        });
        return Object.keys(scaleToScaledSpaceMap)
            .map((key) => +key)
            .map((scale) => ({
                scale: scale,
        ...
        if (vertex.innerSize > 0) {
            ...
            if (vertexGroup.innerScale > innerScale) {
                ...
                vertex.inEdges.forEach((edge) =>
                    this.updateInnerScale(edge.source, innerScale),
                );
                vertex.outEdges.forEach((edge) =>
                    this.updateInnerScale(edge.target, innerScale),
                );
            }
        }
    }
    ...
    private addPositionConstraints(
        lpModel: LPModel,
        layers: Vertex[][],
        vertexDistance: number,
        maxLayerLength: number | undefined,
        ...
        layers.forEach((layer, layerIndex) => {
            ...
            const conFirstPosInLayerLB: string =
                "ConFirstPosInLayerLB" + layerIndex;

            // add lower bound constraint for the first vertex v in a layer
            lpModel.addConstraint(
                conFirstPosInLayerLB,
                firstVertex.topPadding * firstVertex.layerScale +
                    (firstVertex.innerSize / 2) * firstVertex.innerScale,
                null,
                {
                    [varFirstVertexPos]: 1,
                },
            ...
            for (let iV: number = 1, nV: number = layer.length; iV < nV; iV++) {
                ...
                const minDistance: number = getMinScaledVertexPairPosDistance(
                    precessorVertex,
                    vertex,
                    vertexDistance,
                );
                ...
                const varPreVertexPos: string =
                    buildPosVarName(precessorVertex);
                const conMinDistBetweenVertexPos: string =
                    "ConMinDist_" + varPreVertexPos + "_" + varVertexPos;

                lpModel.addConstraint(
                    conMinDistBetweenVertexPos,
                    minDistance,
                    null,
                    {
                        [varVertexPos]: 1,
            ...
            lpModel.addConstraint(
                conNameUB,
                lastVertex.bottomPadding * lastVertex.layerScale +
                    (lastVertex.innerSize / 2) * lastVertex.innerScale,
                null,
                {
                    [varMaxSize]: 1,
                    [varLastVertexPos]: -1,
    ...
    private addStraightLineConstraints(lpModel: LPModel, layers: Vertex[][]) {
        for (const layer of layers) {
            for (const vertex of layer) {
                ...
                const varNonVirtualSourcePos =
                    buildPosVarName(nonVirtualSource);
                const nonVirtualTarget = this.getNonVirtualTarget(vertex);
                const varNonVirtualTargetPos =
                    buildPosVarName(nonVirtualTarget);
                const varVertexPos = buildPosVarName(vertex);
                const varLineDistance = buildLineDistanceVarName(vertex);

                const layerSpan =
                    nonVirtualSource.layerIndex - nonVirtualTarget.layerIndex;
                const spanFactor =
                    (nonVirtualSource.layerIndex - vertex.layerIndex) /
                    layerSpan;
                ...
                lpModel.setObjectiveCoefficient(
                    varLineDistance,
                    OBJ_FUN_LINE_DISTANCE_PENALTY,
                );
            }
        }
    }
    ...
    private addCenterPosConstraints(lpModel: LPModel, layers: Vertex[][]) {
        ...
        for (const layer of layers) {
            for (const vertex of layer) {
                ...
                lpModel.setObjectiveCoefficient(
                    varDistance,
                    totalWeight * OBJ_FUN_CENTER_DISTANCE_PENALTY,
                );
            }
        }
    }
