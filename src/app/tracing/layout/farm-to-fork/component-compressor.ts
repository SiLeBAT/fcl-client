    ...
    targetToSourcesMap.forEach((sources: Vertex[]) => {
      if (sources.length >= 2) {
        ...
        sources.forEach((source, memberIndex) => {
          if (memberIndex > 0) {
            const vertexPairDistance = getMinVertexPairSpecificDistance(
              sources[memberIndex - 1],
              source,
              vertexDistance
            );
        ...
        vertexGroup.innerSize =
          vertexGroup.outerSize -
          vertexGroup.topPadding -
          vertexGroup.bottomPadding;
...
function getSourceToSimpleTargetsMap(
  layer: Vertex[]
): Map<VertexIndex, Vertex[]> {
...
function getTargetToSimpleSourcesMap(
  layer: Vertex[]
): Map<VertexIndex, Vertex[]> {
...
export function decompressSimpleSources(graph: Graph, vertexDistance: number) {
  for (const layer of graph.layers) {
    for (const vertex of layer.filter(
      ...
      const vertexGroup: CompressedVertexGroup =
        vertex as CompressedVertexGroup;
      if (vertexGroup.compressionType === CompressionType.SOURCE_COMPRESSION) {
        ...
        target.inEdges = target.inEdges.filter(
          e => e !== vertexGroup.outEdges[0]
        );
        ...
        let y: number =
          vertexGroup.y - (vertexGroup.innerSize / 2) * vertexGroup.innerScale;
        ...
        for (const source of vertexGroup.compressedVertices) {
          if (source !== vertexGroup.compressedVertices[0]) {
            y +=
              (source.topPadding + source.innerSize / 2) *
              vertexGroup.innerScale;
          }
          source.y = y;
          y +=
            (source.innerSize / 2 + source.bottomPadding + vertexDistance) *
            vertexGroup.innerScale;
      ...
      if (vertexGroup.compressionType === CompressionType.TARGET_COMPRESSION) {
        ...
        for (const target of vertexGroup.compressedVertices) {
          if (target !== vertexGroup.compressedVertices[0]) {
            y +=
              (target.topPadding + target.innerSize / 2) *
              vertexGroup.innerScale;
          }
          target.y = y;
          y +=
            (target.innerSize / 2 + target.bottomPadding + vertexDistance) *
            vertexGroup.innerScale;
