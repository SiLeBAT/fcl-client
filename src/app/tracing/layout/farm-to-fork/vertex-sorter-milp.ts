    ...
    for (const layer of graph.layers) {
        layer.sort((a, b) =>
            a.indexInLayer < b.indexInLayer
                ? lpResult.vars.get(buildXVarName(a, b))! > 0.5
                    ? -1
                    : 1
                : lpResult.vars.get(buildXVarName(b, a))! > 0.5
                  ? 1
                  : -1,
        );

        for (let i: number = layer.length - 1; i >= 0; i--) {
...
function buildCVarName(
    ...
    return (
        "c(" +
        vertexToString(edgeAFrom) +
        "->" +
        vertexToString(edgeATo) +
        "),(" +
        vertexToString(edgeBFrom) +
        "->" +
        vertexToString(edgeBTo) +
        ")"
    );
}
...
function addConstraintsAndObjective(layers: Vertex[][], lpModel) {
    ...
    for (const layer of layers) {
            ...
            for (let j: number = 1; j < k; j++) {
                ...
                for (let i: number = 0; i < j; i++) {
                    ...
                    const conName: string =
                        "C13.23(L:" +
                        layer[k].layerIndex.toString() +
                        "):(" +
                        layer[i].index.toString() +
                        "," +
                        layer[j].index.toString() +
                        "," +
                        layer[k].index.toString() +
                        ")";
        ...
        for (let iT2: number = targets.length - 1; iT2 > 0; iT2--) {
            for (let iT1: number = 0; iT1 < iT2; iT1++) {
                ...
                const sourcesFromTargetI: Vertex[] = targetI.inEdges.map(
                    (e) => e.source,
                );
                const sourcesFromTargetK: Vertex[] = targetK.inEdges.map(
                    (e) => e.source,
                );
    ...
    const objective = {};
    for (const varC of crossingVars) {
        objective[varC] = 1;
    }
    for (const varN of neighbourVars) {
        objective[varN] = -1 / neighbourVars.length / 2;
    }
