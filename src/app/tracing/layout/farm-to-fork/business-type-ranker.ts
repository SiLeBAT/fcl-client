...
export class BusinessTypeRanker {
  ...
  private init(
    ...
    for (const s of sinkTypes) {
      this.isSink[this.add(s)] = true;
    }
    for (const s of sourceTypes) {
      this.isSource[this.add(s)] = true;
    }
    for (const orderedChain of typeOrderings) {
      for (const s of orderedChain) {
        this.add(s);
      }
    }
    ...
    for (let i: number = typeCount - 1; i >= 0; i--) {
      if (this.isSink[i] != null) {
        this.isSink[i] = false;
      }
      if (this.isSource[i] != null) {
        this.isSource[i] = false;
      }
    }
    // init rankMatrix
    ...
    for (const sinkCode of sinkCodes) {
      for (let i: number = typeCount - 1; i >= 0; i--) {
        if (!this.isSink[i]) {
          this.rankMatrix[sinkCode][i] = true;
        }
      }
    }
  ...
  compareRankingByCode(
    typeA: number | undefined,
    typeB: number | undefined
  ): number | undefined {
    if (typeA != null) {
      if (typeB != null) {
        if (this.rankMatrix[typeA][typeB]) {
          return -1;
        }
        if (this.rankMatrix[typeB][typeA]) {
          return 1;
        }
        return undefined;
      } else {
        // type B is unknown
        if (this.isSink[typeA]) {
          return -1;
        }
        if (this.isSource[typeA]) {
          return 1;
        }
        return undefined;
      }
      ...
      // typeA is unkown
      if (this.isSink[typeB]) {
        return 1;
      }
      if (this.isSource[typeB]) {
        return -1;
      }
      return undefined;
    }
  ...
  compareRankingByType(
    typeA: string | undefined,
    typeB: string | undefined
  ): number | undefined {
    const codeA: number | undefined =
      typeA && this.indexMap.has(typeA) ? this.indexMap.get(typeA)! : undefined;
    const codeB: number | undefined =
      typeB && this.indexMap.has(typeB) ? this.indexMap.get(typeB)! : undefined;
  ...
  getBusinessTypeCode(type: string | undefined): number | undefined {
    return type && this.indexMap.has(type)
      ? this.indexMap.get(type)!
      : undefined;
  }

  private add(type: string): number {
    if (this.indexMap.has(type)) {
      return this.indexMap.get(type)!;
    }
