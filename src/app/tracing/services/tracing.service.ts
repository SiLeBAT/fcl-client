...
export class TracingService {
  ...
  private traceDelivery(
    ...
    if (forward && !delivery.killContamination) {
      ...
      if (!targetStation.killContamination) {
        data
          .getDelById(this.getForwardDeliveries(data, targetStation, delivery))
    ...
    if (backward) {
      ...
      if (!sourceStation.killContamination) {
        sourceStation.backward = true;
        data
          .getDelById(this.getBackwardDeliveries(data, sourceStation, delivery))
          .filter(backDel => !backDel.invisible && !backDel.backward)
          .forEach(backDel => {
  ...
  private traceStation(
    ...
    if (forward && !station.killContamination) {
      data
        .getDelById(station.outgoing)
        .filter(d => !d.invisible && !d.forward && !d.killContamination)
        .forEach(delivery => {
          delivery.forward = true;
    ...
    if (backward) {
      data
        .getDelById(station.incoming)
        .filter(d => !d.invisible && !d.backward && !d.killContamination)
        .forEach(delivery => {
          delivery.backward = true;
