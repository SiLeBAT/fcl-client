...
export class HighlightingService {
  ...
  applyVisibilities(state: DataServiceInputState, data: DataServiceData) {
    ...
    data
      .getStatById(state.highlightingSettings.invisibleStations)
      .filter(s => s !== null)
      .forEach(s => {
        s.invisible = true;
        s.expInvisible = true;
    ...
    data
      .getDelById(state.highlightingSettings.invisibleDeliveries)
      .forEach(d => {
        d.invisible = true;
        d.expInvisible = true;
      });
      ...
      return (item: StationOrDeliveryData) =>
        ppConditionGroups.some(ppConditionGroup =>
          ppConditionGroup.every(ppCondition =>
            ppCondition.isValid(
              this.getPropertyValueFromElement(item, ppCondition.property)
            )
          )
        );
    }
  ...
  private getRuleIdToIsActiveMap(
    ...
    hrules.forEach(
      rule => (result[rule.id] = (effElementsStats.counts[rule.id] || 0) > 0)
    );
    return result;
  }
  ...
  private getLegendInfo(activeHighlightings: {
    ...
    const deliveryRulesToDisplay = this.enabledDelHRules.filter(
      rule => rule.showInLegend && activeHighlightings.deliveries[rule.id]
    );
    ...
    return uniqueDisplayEntries
      ...
      .filter(
        rule =>
          rule.deliveryColor || rule.shape !== undefined || rule.stationColor
      );
  }
  ...
  private getCommonHighlightingInfo<
    ...
    K extends T extends StationData
      ? StationHighlightingRule
      : DeliveryHighlightingRule,
  >(fclElement: T, highlightingRules: K[]): {label: string; color: Color[]} {
    ...
    const color = removeNullish(highlightingRules.map(rule => rule.color));
