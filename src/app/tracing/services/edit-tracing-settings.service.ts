...
export class EditTracingSettingsService {
  ...
  getClearKillContaminationsPayload(
    ...
    const updateFun = <T extends ElementTracingSettings>(x: T) => ({
      ...x,
  ...
  getMarkElementsAsOutbreakPayload(
    ...
    const updateFun = <T extends ElementTracingSettings>(x: T) => ({
      ...x,
      outbreak: options.outbreak,
      weight: weight,
    });

    const newStatSettings = options.stationIds
      ? this.getNewElementsSettings(
  ...
  getClearCrossContaminationPayload(
    tracingSettings: TracingSettings
  ): SetTracingSettingsPayload {
    ...
    return {
      tracingSettings: {
        ...
        stations: tracingSettings.stations.map(s => ({
          ...s,
          crossContamination: idSet[s.id]
            ? crossContamination
            : s.crossContamination,
        })),
      },
    };
  }
  ...
  getSetKillContaminationPayload(
    ...
    if (options.stationIds) {
      ...
      const updateFilter = (s: StationTracingSettings) =>
        statIds.has(s.id) && s.killContamination !== options.killContamination;
      const updateFun = (s: StationTracingSettings) =>
        getUpdatedObject(s, {
          killContamination: options.killContamination,
          crossContamination:
            s.crossContamination && !options.killContamination,
        });
      newTracingSettings.stations = this.getNewElementsSettings(
        tracingSettings.stations,
        updateFilter,
        updateFun
      );
    ...
    if (options.deliveryIds) {
      ...
      const updateFilter = (s: DeliveryTracingSettings) =>
        delIds.has(s.id) && s.killContamination !== options.killContamination;
      const updateFun = (s: DeliveryTracingSettings) =>
        getUpdatedObject(s, {killContamination: options.killContamination});
      newTracingSettings.deliveries = this.getNewElementsSettings(
        tracingSettings.deliveries,
        updateFilter,
        updateFun
      );
  ...
  private getNewTracing<
    T extends StationTracingSettings | DeliveryTracingSettings,
  >(oldTracing: T[], newInvIds: string[]): T[] {
  ...
  private setElementsObservedType<
    T extends StationTracingSettings | DeliveryTracingSettings,
  >(elements: T[], ids: string[], observedType: ObservedType): T[] {
