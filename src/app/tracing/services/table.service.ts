...
export class TableService {
  ...
  private getDeliveryRows(
    ...
    return (deliveryIds ? data.getDelById(deliveryIds) : data.deliveries).map(
      delivery => {
        const row: TableRow = {
          id: delivery.id,
          highlightingInfo: {
        ...
        delivery.properties.forEach(prop => (row[prop.name] = prop.value));
  ...
  private decamelize(str: string): string {
    ...
    return str
      ...
      .replace(
        /^([a-z]+)/g,
        (match: string, p1: string) => p1.charAt(0).toUpperCase() + p1.slice(1)
      )
      .replace(
        /(^\_chargen\.)(.*)/gi,
