...
export class TracingEffects {
            ...
            this.actions$.pipe(
                ...
                mergeMap(([action, state]) => {
                    ...
                    if (station) {
                        ...
                        for (const delivery of data.getDelById(
                            station.incoming,
                        )) {
    ...
    focusStationSSA$ = createEffect(
        () =>
            this.actions$.pipe(
                ...
                withLatestFrom(
                    this.store.pipe(
                        select(tracingSelectors.selectSharedGraphState),
                    ),
                ),
                mergeMap(([action, state]) => {
                    const focusStationId = action.payload.stationId;
                    const focuseNodeId =
                        this.graphService.getNodeIdFromStationId(
                            focusStationId,
                            state,
                        );
        ...
        () =>
            this.actions$.pipe(
                ...
                mergeMap(([action, state]) => {
                    ...
                    const focuseEdgeId =
                        this.graphService.getEdgeIdFromDeliveryId(
                            focusDeliveryId,
                            state,
                        );
                    ...
                    if (focuseEdgeId !== null) {
                        if (state.selectedElements.stations.length > 0) {
                            this.store.dispatch(
                                new tracingStateActions.SetSelectedStationsSOA({
                                    stationIds: [],
                                }),
                            );
                        }
                    ...
                    if (deliveryIds.length === 1) {
                        ...
                        if (delivery) {
                            const dialogData: DeliveryPropertiesData = {
                                ...
                                originalSource:
                                    data.statMap[delivery.originalSource],
                                originalTarget:
                                    data.statMap[delivery.originalTarget],
                                deliveryColumns:
                                    this.tableService.getDeliveryColumnSets(
                                        state,
                                        data,
                                        false,
                                    ).columns,
        ...
        this.actions$.pipe(
            ...
            mergeMap(([action, state]) => {
                try {
                    ...
                    if (payload) {
                        return of(
                            new tracingStateActions.SetTracingSettingsSOA(
                                payload,
                            ),
                        );
                    }
    ...
    clearTrace$ = createEffect(() =>
        this.actions$.pipe(
            ...
            mergeMap(([action, state]) => {
                try {
                    const payload =
                        this.editTracSettingsService.getClearTracePayload(
                            state,
                        );
                    if (payload) {
                        return of(
        ...
        this.actions$.pipe(
            ...
            mergeMap(([action, state]) => {
                try {
                    ...
                    if (payload) {
                        return of(
                            new tracingStateActions.SetInvisibleElementsSOA(
                                payload,
                            ),
                        );
                    }
    ...
    clearInvisiblities$ = createEffect(() =>
        this.actions$.pipe(
            ...
            withLatestFrom(
                this.store.pipe(
                    select(tracingSelectors.selectHighlightingSettings),
                ),
            ),
            mergeMap(([action, state]) => {
                try {
                    ...
                    if (payload) {
                        return of(
                            new tracingStateActions.SetHighlightingSettingsSOA(
                                payload,
                            ),
                        );
                    }
    ...
    clearOutbreaks$ = createEffect(() =>
        this.actions$.pipe(
            ...
            mergeMap(([action, state]) => {
                try {
                    const payload =
                        this.editTracSettingsService.getClearOutbreaksPayload(
                            state,
                            action.payload,
                        );
                    if (payload) {
            ...
            this.actions$.pipe(
                ...
                mergeMap(([action, state]) => {
                    try {
                        ...
                        if (
                            oldStationPositionsAreEmpty &&
                            !payloadPositionsAreEmpty
                        ) {
                            this.store.dispatch(
