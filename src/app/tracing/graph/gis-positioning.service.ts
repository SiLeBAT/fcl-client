...
export class GisPositioningService {
    ...
    private updateCache(): void {
        this.cachedPositioningData = {
            ...
            unknownLatLonRect:
                this.boundaryNodeIds.length > 0 ||
                this.boundaryGhostNodeIds.length > 0
    ...
    private initKnownStationModelPositions(): void {
        ...
        for (const station of this.graphData!.stations.filter(
            UIUtils.hasGisInfo,
        )) {
    ...
    private createDefaultPosition(): Position {
        return {
            ...
            y:
                (this.outerBoundaryRect!.top + this.outerBoundaryRect!.bottom) /
                2,
        };
    }
    ...
    private setUnknownNodeModelPositions(): void {
        ...
        const idsOfConnectedNodesWOPos =
            this.getNodesWoPosConnectedWithNodeWPos(
        ...
        this.graphData!.nodeData.filter(
            ...
            (n) =>
                (this.nodeModelPositions[n.id] = this.createDefaultPosition()),
        );
    }
    ...
    private createNeighbourHoodMap(
        ...
        for (const edge of graphData.edgeData.filter(
            (e) => e.source !== e.target,
        )) {
