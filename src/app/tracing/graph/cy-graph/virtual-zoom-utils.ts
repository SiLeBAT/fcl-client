...
export function vpToStr(viewPort: Layout): string {
    return (
        "z: " +
        nToStr(viewPort.zoom) +
        ", px: " +
        nToStr(viewPort.pan.x) +
        ", py: " +
        nToStr(viewPort.pan.y)
    );
}

export function sizeToStr(size: Size): string {
    return "w: " + nToStr(size.width) + ", h: " + nToStr(size.height);
}
...
export function getExtendedTargetIncludingViewPort(
    ...
    let rendTargetRect = getRenderedRect(
        modelTargetRect,
        targetMargin,
        refViewPort,
    );
    // this is the rect we want the sub graph rect to be inside
    let rendRefRect = createRect(
        0,
        0,
    ...
    if (isRectWithinRect(rendTargetRect, rendRefRect)) {
        ...
        // determine zoom
        const spaceWoMargin = getSizeWithoutMargin(
            availableSpace,
            aggMargins(refMargin, targetMargin),
        );
        ...
        minZoom = Math.max(
            minZoom,
            getMinSizeRatio(spaceWoMargin, modelUnionRect),
        );
        ...
        const zoomTolerance =
            1 / Math.max(availableSpace.width, availableSpace.height);
        do {
            viewport.zoom = (maxZoom + minZoom) / 2;
            rendTargetRect = getRenderedRect(
                modelTargetRect,
                targetMargin,
                viewport,
            );
        ...
        if (rendUnionRect.width <= availableSpace.width) {
            ...
            viewport.pan.x +=
                -rendUnionRect.left +
                (availableSpace.width - rendUnionRect.width) / 2;
        } else if (availableSpace.width <= rendTargetRect.width) {
            // SubGraph is wider than viewport (minZoom constraint)
            viewport.pan.x -=
                rendTargetRect.left +
                (rendTargetRect.width - availableSpace.width) / 2;
        } else {
            // rendered union is wider than viewport
            viewport.pan.x =
                -Math.max(
                    0,
                    modelTargetRect.right * viewport.zoom +
                        targetMargin.right -
                        availableSpace.width,
                ) +
                +Math.max(
                    0,
                    -(modelTargetRect.left * viewport.zoom - targetMargin.left),
                );
        }
        if (rendUnionRect.height <= availableSpace.height) {
            ...
            viewport.pan.y +=
                -rendUnionRect.top +
                (availableSpace.height - rendUnionRect.height) / 2;
        } else if (availableSpace.height <= rendTargetRect.height) {
            // SubGraph is heigher than viewport (minZoom constraint)
            viewport.pan.y -=
                rendTargetRect.top +
                (rendTargetRect.height - availableSpace.height) / 2;
        } else {
            // rendered union is wider than viewport
            viewport.pan.y =
                -Math.max(
                    0,
                    modelTargetRect.bottom * viewport.zoom +
                        targetMargin.bottom -
                        availableSpace.height,
                ) +
                +Math.max(
                    0,
                    -(modelTargetRect.top * viewport.zoom - targetMargin.top),
                );
        }
...
function getMinSizeRatio(size1: Size, size2: Size): number {
    return Math.min(size1.width / size2.width, size1.height / size2.height);
}

export function getRenderedRect(
    modelRect: BoundaryRect,
    modelMargin: Margin,
    viewport: Layout,
): BoundaryRect {
    const left =
        modelRect.left * viewport.zoom + viewport.pan.x - modelMargin.left;
    const right =
        modelRect.right * viewport.zoom + viewport.pan.x + modelMargin.right;
    const top =
        modelRect.top * viewport.zoom + viewport.pan.y - modelMargin.top;
    const bottom =
        modelRect.bottom * viewport.zoom + viewport.pan.y + modelMargin.bottom;
    return {
        left: left,
        right: right,
        top: top,
