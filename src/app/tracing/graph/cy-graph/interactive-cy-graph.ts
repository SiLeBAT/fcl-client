...
export class InteractiveCyGraph extends CyGraph {
  ...
  zoomToPercentage(value: number): void {
    if (this.cy) {
      this.zoomTo(
        Math.exp((value / 100) * Math.log(this.maxZoom / this.minZoom)) *
          this.minZoom
      );
    }
  }
  ...
  getLayoutOptions(nodesToLayout: NodeId[]): LayoutOption[] {
    const isNodeCountSufficient =
      nodesToLayout.length >= InteractiveCyGraph.MIN_RELAYOUTING_NODE_COUNT;
    ...
    return knownLayoutManagerNames.map(layoutManagerName => ({
      ...
      disabled:
        ...
        (!areAllNodesGoingToBeLayouted &&
          layoutManagersNotSupportingSubsets.indexOf(layoutManagerName) >= 0),
    }));
  }
  ...
  updateGraph(graphData: GraphData, styleConfig: StyleConfig): void {
    if (this.cy) {
      ...
      const updateSelection =
        !updateNodes &&
        !_.isEqual(oldData.selectedElements, graphData.selectedElements);
  ...
  private arePositionsDifferent(
    ...
    if (graphData1.nodePositions === graphData2.nodePositions) {
      ...
      return graphData1.nodeData.some(n => {
        ...
        return (
          this.getRelPosDiff(pos1, pos2) > InteractiveCyGraph.POSITION_TOLERANCE
        );
      });
    }
  }
