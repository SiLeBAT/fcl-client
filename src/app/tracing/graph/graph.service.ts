...
export class GraphService {
  ...
  static updateRelZindexOfEdges(edgeData: CyEdgeData[]) {
    ...
    edgeData.forEach(
      edge => (eScore[edge.id] = Math.max(...edge.deliveries.map(d => d.score)))
    );
  ...
  static updateAbsZindex(nodeData: CyNodeData[], edgeData: CyEdgeData[]) {
    ...
    edgeData.forEach(
      e => (e.zindex = e.relZindex + (e.selected ? indexOffsetOfSelEdge : 0))
    );
  }
          ...
          for (const deliveries of deliveryGroups) {
            if (deliveries.length === 1) {
              ...
              edgeData.push({
                ...
                ...this.getColorInfo(
                  this.mergeColors(
                    deliveries.map(d => d.highlightingInfo!.color)
                  ),
                  GraphService.DEFAULT_EDGE_COLOR
                ),
                source: sourceDataId,
  ...
  private createMergedLabelWoPrefix(deliveries: DeliveryData[]): string {
    ...
    const labels: string[] = _.uniq(deliveries.map(d => this.createLabel(d)));
  ...
  private createGhostEdgeDataFromNode(
    ...
    ghostEdgeData = concat(
      ...deliveriesPerNodePair.map(deliveriesForNodePair => {
        ...
        const nodePairEdges = deliveryGroups.map((deliveryGroup, gIndex) =>
  ...
  private applyState(state: SharedGraphState) {
    ...
    const edgeCreationMightBeRequired =
      ...
      (this.cachedState!.mergeDeliveriesType !== state.mergeDeliveriesType ||
        (state.mergeDeliveriesType === MergeDeliveriesType.MERGE_LABEL_WISE &&
          highlightingPropsWereUpdated));

    const propUpdateRequired =
      tracingPropsWereUpdated || highlightingPropsWereUpdated;

    const nodePropsUpdateRequired = !nodeCreationRequired && propUpdateRequired;
    ...
    if (maintainOffGraphSelection) {
      ...
      selectedElements.deliveries = concat(
        ...
        Utils.getStringArrayDifference(
          ...
          concat(
            ...graphServiceData.edgeData.map(e => e.deliveries.map(d => d.id))
          )
        )
      );
    }
