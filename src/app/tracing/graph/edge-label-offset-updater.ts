...
export class EdgeLabelOffsetUpdater {
  ...
  updateGhostEdges() {
    if (this.cy !== null) {
      this.cy.batch(() =>
        this.updateEdgeLabelOffsets(this.cy!.edges('.ghost-element'))
      );
    }
  }
  ...
  private updateEdgeLabelOffsets(edges: CyElementCollection<CyEdge>) {
    ...
    edges.forEach(edge => {
      ...
      const offset: number =
        fontSize / 2.0 + +strWidth.substr(0, strWidth.length - 2) / 2.0;
  ...
  private updateEdgeLabelOffset(edge: CyEdge, offset: number): void {
    ...
    if (dEy === 0) {
      ...
      const adTx = Math.sqrt(
        (offset * offset) / (1 + a_eX_to_eY_ratio * a_eX_to_eY_ratio)
      );
