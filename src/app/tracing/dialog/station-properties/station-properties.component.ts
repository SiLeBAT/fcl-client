...
export class StationPropertiesComponent implements OnInit, OnDestroy {
  ...
  private initProperties(station: StationData, columns: TableColumn[]): void {
    ...
    this.conditionalNotListedPropsMap.forEach(
      (isHiddenFun, propId) => (hiddenProps[propId] = isHiddenFun(station))
    );
  ...
  ngOnInit() {
    if (this.height != null) {
      ...
      this.connectLine = g
        ...
        .attr('fill', 'none')
        .attr('stroke', Utils.colorToCss(Constants.DEFAULT_STROKE_COLOR))
  ...
  private addNodes() {
    const updateColor = (
      ...
      nodes
        ...
        .attr(
          'fill',
          Utils.colorToCss(
            hovered ? Constants.HOVER_FILL_COLOR : Constants.DEFAULT_FILL_COLOR
          )
        )
        .attr(
          'stroke',
    ...
    const initRectAndText = (
      ...
      text
        ...
        .attr('font-size', '14px')
        .text(d =>
          d.lot != null ? d.name + ' (' + d.lot + ')' : d.name ?? ''
        );
      text
    ...
    newEdges
      .on('mouseover', (event, edge: EdgeDatum) => {
        if (this.selected == null) {
          ...
          this.hoverDeliveries(
            concat(edge.source.deliveryIds, edge.target.deliveryIds)
          );
        }
      })
