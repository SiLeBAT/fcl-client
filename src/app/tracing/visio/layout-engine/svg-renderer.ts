...
export class SvgRenderer {
    ...
    static renderReport(report: VisioReport): string {
        return (
            ...
            this.renderConnectors(
                report.graph.connectors,
                this.getPortPositions(report.graph.elements),
            ) +
            "</svg>"
        );
    }
    ...
    private static getSortedBoxes(boxes: VisioBox[]): VisioBox[] {
        ...
        result.sort((b1, b2) =>
            Math.min(
                b2.size.width - b1.size.width,
                b2.size.height - b1.size.height,
            ),
        );
        return result;
    }
        ...
        for (const box of boxes) {
            for (const port of box.ports) {
                result.set(port.id, {
                    x:
                        box.position.x +
                        box.size.width * port.normalizedPosition.x,
                    y:
                        box.position.y +
                        box.size.height * port.normalizedPosition.y,
                });
            }
            this.getPortPositions(box.elements).forEach(
                (value: Position, key: string) => {
    ...
    private static renderLabel(box: VisioBox): string {
        ...
        const pos: Position = {
            x:
                label.relPosition.x +
                box.position.x +
                (centerAlign ? label.size.width / 2 : 0),
            y: label.relPosition.y + box.position.y,
        };
        let svg =
            '<text alignment-baseline="top" ' +
            'text-anchor="' +
            (centerAlign ? "middle" : "start") +
            '" ' +
            'x="' +
            pos.x +
            '" ' +
    ...
    private static renderConnectors(
        ...
        portPositions: Map<string, Position>,
    ): string {
        let svg = "";
