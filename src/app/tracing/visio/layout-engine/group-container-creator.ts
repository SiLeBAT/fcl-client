...
export class GroupContainerCreator {
  ...
  private createGroup(label: VisioLabel, cells: GridCell[]): VisioBox {
    ...
    cells.forEach(cell => {
      ...
      if (box) {
        box.relPosition = {
          x:
            this.columnLeft[cell.column] -
            this.columnLeft[minColumn] +
            (this.columnWidth[cell.column] -
              GraphSettings.GRID_MARGIN -
              box.size.width) /
              2,
          y:
            this.rowTop[cell.row] -
            this.rowTop[minRow] +
            GraphSettings.GROUP_MARGIN +
            GraphSettings.GROUP_HEADER_HEIGHT,
        };
        elements.push(box);
      }
    });
    ...
    label.relPosition = {
      x:
        this.columnLeft[labelCell.column] -
        this.columnLeft[minColumn] +
        GraphSettings.GROUP_MARGIN,
      y:
        this.rowTop[labelCell.row] -
        this.rowTop[minRow] +
        GraphSettings.GROUP_MARGIN,
    };
    ...
    const shape = this.createGroupShape(cells, {
      row: minRow,
      column: minColumn,
    });
  ...
  private setRowAndColumnHeights(
    cellGroups: {label: VisioLabel; cells: GridCell[]}[],
    graphLayers: GraphLayer[]
  ) {
    this.groupHeaderHeight =
      Math.max(...cellGroups.map(g => g.label.size.height)) +
    ...
    const rowHeight: number[] = this.boxGrid.map(
      boxRow =>
        Math.max(...boxRow.map(box => (box === null ? 0 : box.size.height))) +
        this.topMargin +
        this.bottomMargin
    );
    ...
    this.columnWidth = mapColumns(
      this.boxGrid,
      boxColumn =>
        Math.max(...boxColumn.map(box => (box ? box.size.width : 0))) +
        2 * this.xMargin
    );
  ...
  private createGroupShape(
    ...
    const reTrans: (GridCell) => GridCell = c => ({
      row: c.row + rowOffset,
      column: c.column + columnOffset,
    });
    ...
    return {
      outerBoundary: this.getRelativePolygon(
        this.convertCellPolygon(
          outerBoundary.map(c => reTrans(c)),
          false
        ),
        refPos
      ),
      holes: holes.map(h =>
        this.getRelativePolygon(
          this.convertCellPolygon(
            h.map(c => reTrans(c)),
            true
          ),
  ...
  private parseOuterBoundary(matrix: boolean[][], polygon: CellPolygon) {
    ...
    if (polygon.length === 1) {
      if (
        matrix[lastCell.row][lastCell.column + 1] ===
        matrix[lastCell.row][lastCell.column]
      ) {
        polygon.push({row: lastCell.row, column: lastCell.column + 1});
        this.parseOuterBoundary(matrix, polygon);
      } else if (
        matrix[lastCell.row + 1][lastCell.column] ===
        matrix[lastCell.row][lastCell.column]
      ) {
      ...
      if (
        lastCell.column !== polygon[0].column ||
        lastCell.row !== polygon[0].row || // not the start cell
        ...
        const leftCell: GridCell = {
          ...
          column:
            lastCell.column + lastMove.row * (1 - Math.abs(lastMove.column)),
        };
        if (
          matrix[lastCell.row][lastCell.column] ===
          matrix[leftCell.row][leftCell.column]
        ) {
          ...
          // check straight
          const straightCell = {
            row: lastCell.row + lastMove.row,
            column: lastCell.column + lastMove.column,
          };
          if (
            matrix[lastCell.row][lastCell.column] ===
            matrix[straightCell.row][straightCell.column]
          ) {
            polygon.pop();
            ...
            const rightCell = {
              row:
                lastCell.row + lastMove.column * (1 - Math.abs(lastMove.row)),
              column:
                lastCell.column -
                lastMove.row * (1 - Math.abs(lastMove.column)),
            };
            if (
              matrix[lastCell.row][lastCell.column] ===
              matrix[rightCell.row][rightCell.column]
            ) {
    ...
    if (polygon.length === 1) {
      ...
      for (let i = 0, n = polygon.length; i < n; i++) {
        const lastMove: GridCell =
          i > 0
            ? {
                row: polygon[i].row - polygon[i - 1].row,
                column: polygon[i].column - polygon[i - 1].column,
              }
            : {row: -1, column: 0};
        const nextMove: GridCell =
          i < n - 1
            ? {
                row: polygon[i + 1].row - polygon[i].row,
                column: polygon[i + 1].column - polygon[i].column,
              }
            : {
                row: polygon[1].row - polygon[0].row,
                column: polygon[1].column - polygon[0].column,
              };

        if (this.areMovesReverse(lastMove, nextMove)) {
          // U-turn
          const tempMove: GridCell = {
            row: (lastMove.column - nextMove.column) / 2,
            column: (nextMove.row - lastMove.row) / 2,
          };

          result.push(
            this.convertCellToPoint(polygon[i], lastMove, tempMove, margin)
          );
          result.push(
            this.convertCellToPoint(polygon[i], tempMove, nextMove, margin)
          );
        } else {
          // simple turn
          result.push(
            this.convertCellToPoint(polygon[i], lastMove, nextMove, margin)
          );
        }
      }
      return result;
  ...
  private areMovesReverse(moveA: GridCell, moveB: GridCell): boolean {
    return (
      Math.sign(moveA.column) === -Math.sign(moveB.column) &&
      Math.sign(moveA.row) === -Math.sign(moveB.row)
    );
  }

  private convertCellToPoint(
    cell: GridCell,
    lastMove: GridCell,
    nextMove: GridCell,
    margin: number
