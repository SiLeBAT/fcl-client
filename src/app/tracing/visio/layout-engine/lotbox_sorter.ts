...
export class LotBoxSorter {
    ...
    constructor(
        ...
        this.portToConnectedPositionsMap =
            this.createPortToConnectedPositionsMap(portToConnectedPortsMap);
    }
    ...
    private getReferencePosition(boxes: VisioBox[]): Position {
        ...
        const maxX = Math.max(
            ...boxes.map((b) => b.position!.x + b.size.width),
        );
        return {
            x: minX + (maxX - minX) / 2,
    ...
    private getWeight(box: VisioBox, fromPortPosition: Position): number {
        const positions = this.portToConnectedPositionsMap.get(
            box.ports[0].id,
        )!;
    ...
    private createPortToConnectedPortsMap(
        ...
        connectors.forEach((connector) => {
            ...
            portId2ConnectedPortIds
                .get(connector.fromPort)!
                .push(connector.toPort);
            portId2ConnectedPortIds
                .get(connector.toPort)!
                .push(connector.fromPort);
        });
    ...
    private createPortToPositionMap(
        ...
        portToBoxMap.forEach((box, port) => {
            result.set(port.id, {
                ...
                y:
                    box.position!.y +
                    port.normalizedPosition.y * box.size.height,
            });
        });
