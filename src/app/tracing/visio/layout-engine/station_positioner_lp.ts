...
function addBoxPorts(
    ports: { id: string; port: BoxPort }[],
    visioBox: VisioBox,
    unnormalizePos: (x: number) => number,
) {
    ...
    visioBox.elements.forEach((element) =>
        addBoxPorts(
            ...
            (x: number) =>
                unnormalizePos(0) +
                element.relPosition.x +
                element.size.width * x,
        ),
    );
}
...
function createBoxToGroupIndexMap(
    stationBoxToBoxMap: Map<VisioBox, Box>,
    groups: { label: string; boxes: VisioBox[] }[],
): Map<Box, number> {
    const result: Map<Box, number> = new Map();
    groups.forEach((group, groupIndex) => {
        group.boxes.forEach((stationBox) =>
            result.set(stationBoxToBoxMap.get(stationBox)!, groupIndex),
        );
    });
    return result;
}
...
function createLinks(
    idToPortMap: Map<string, BoxPort>,
...
function constructBoxModel(
    ...
    const boxToGroupIndexMap = createBoxToGroupIndexMap(
        statBoxToBoxMap,
        groups,
    );
    const isGroupEqual = (a: Box, b: Box) =>
        boxToGroupIndexMap.get(a) === boxToGroupIndexMap.get(b);
    return {
        boxes: boxes,
...
function addNeighbourConstraints(
    boxModel: BoxModel,
    lpModel: LPModel,
    intraGroupDistance: number,
    interGroupDistance: number,
...
function addBoundaryContraints(
    boxModel: BoxModel,
    lpModel: LPModel,
    varNameWidth: string,
) {
    boxModel.boxes.forEach((boxRow, rowIndex) => {
        if (boxRow.length > 0) {
            // Box >= 0
            lpModel.addConstraint("BCL" + rowIndex, 0, null, {
                [boxRow[0].id]: 1,
            });
...
export function improvePositions(
    ...
    const lpModel = constructLPModel(
        boxModel,
        intraGroupDistance,
        interGroupDistance,
    );
    ...
    for (let r = 0; r < stationBoxes.length; r++) {
        for (let c = 0; c < stationBoxes[r].length; c++) {
            stationBoxes[r][c].relPosition.x = lpResult.vars.get(
                boxModel.boxes[r][c].id,
            )!;
            stationBoxes[r][c].relPosition.y = stationBoxes[r][c].position!.y;
            stationBoxes[r][c].position!.x = lpResult.vars.get(
                boxModel.boxes[r][c].id,
            )!;
        }
    }
