...
export class FilterService {
    ...
    private readonly OPERATION_TYPE_TO_FUNCTION_MAP: {
        ...
        [ExtendedOperationType.REGEX_EQUAL_IGNORE_CASE]: this
            .isRowValueEqualToIgnoreCaseRegex,
        [ExtendedOperationType.REGEX_NOT_EQUAL]: this.isRowValueUnequalToRegex,
        [ExtendedOperationType.REGEX_NOT_EQUAL_IGNORE_CASE]: this
        ...
        const filteredRows = unfilteredRows.filter((row) =>
            filters.every((filterElem: Filter | ComplexFilter) => {
                if (
                    ...
                    (filterElem as ComplexFilter).filterText ===
                    FilterService.COMPLEX_FILTER_NAME
                ) {
                    const complexFilterConditions: ComplexFilterCondition[] = (filterElem as ComplexFilter)
                        .filterConditions;
                    if (complexFilterConditions.length === 0) {
                        return true;
                    } else {
                        return this.complexFilterRow(
                            row,
    ...
    private complexFilterRow(
        row: any,
        filterConditions: ComplexFilterCondition[]
    ): boolean {
        const junktors: string[] = [];
        const rowResult: boolean = filterConditions
            .map((filterElement: ComplexFilterCondition) => {
                const filterProperty: string = filterElement.property;
                const operation: ExtendedOperationType =
                    filterElement.operation;
                ...
                if (!func) {
                    throw new Error(
                        `ExtendedOperationType ${operation} is not yet implemented for complex filter`
                    );
                }
    ...
    private isRowValueLessThanFilterValue(
        filterValue: ValueType,
        rowValue: ValueType
    ): boolean {
        let result: boolean = false;
        ...
        if (rowValue === undefined || rowValue === null) {
            ...
            if (rowValueType === 'boolean') {
                ...
                result =
                    (filterValue as string).localeCompare(rowValue as string) >
                    0;
            } else if (rowValueType === 'number') {
                if (!isNaN(filterValue as any)) {
                    result = (rowValue as number) < +filterValue;
    ...
    private isRowValueGreaterThanFilterValue(
        filterValue: ValueType,
        rowValue: ValueType
    ): boolean {
        let result: boolean = false;
    ...
    private isRowValueEqualToRegex(
        filterValue: ValueType,
        rowValue: ValueType
    ): boolean {
        let result: boolean = false;
        ...
        if (rowValue === undefined) {
            ...
            const strValue =
                typeof rowValue === 'string' ? rowValue : rowValue.toString();
            if (filterValue === '') {
                result = strValue === '';
            } else {
    ...
    private isRowValueEqualToIgnoreCaseRegex(
        filterValue: ValueType,
        rowValue: ValueType
    ): boolean {
        let result: boolean = false;
    ...
    private isRowValueUnequalToRegex(
        filterValue: ValueType,
        rowValue: ValueType
    ): boolean {
        let result: boolean = false;
    ...
    private isRowValueUnequalToIgnoreCaseRegex(
        filterValue: ValueType,
        rowValue: ValueType
    ): boolean {
        let result: boolean = false;
