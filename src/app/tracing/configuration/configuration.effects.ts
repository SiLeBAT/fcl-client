      ...
      this.actions$.pipe(
        ...
        mergeMap(action => {
          ...
          const dialogData: DialogSelectData = {
            ...
            favouriteOptions:
              action.payload.favouriteColumns.map(mapColumnToOption),
      ...
      this.actions$.pipe(
        ...
        mergeMap(([action, state]) => {
          ...
          const stationRuleToDelete =
            state.stations.find(rule => rule.id === deleteRuleId) || null;
          ...
          if (ruleToDelete !== null) {
            const xPos = (action.payload.deleteRequestData.xPos - 350)
              .toString(10)
              .concat('px');
            const yPos = (action.payload.deleteRequestData.yPos - 140)
              .toString(10)
              .concat('px');
                ...
                result => {
                  if (result === true) {
                    if (stationRuleToDelete !== null) {
                      const newRules = this.editHighlightingService.removeRule(
                        state.stations,
                        deleteRuleId
                      );
                      this.store.dispatch(
                        new SetStationHighlightingRulesSOA({rules: newRules})
                      );
                    } else {
                      const newRules = this.editHighlightingService.removeRule(
                        state.deliveries,
                      ...
                      this.store.dispatch(
                        new SetDeliveryHighlightingRulesSOA({rules: newRules})
                      );
                    }
                  }
                },
