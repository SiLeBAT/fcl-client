...
import {
    ...
    DialogYesNoData,
} from "../dialog/dialog-yes-no/dialog-yes-no.component";
...
export class ConfigurationEffects {
    ...
    selectFilterTableColumnsMSA$ = createEffect(
        () =>
            this.actions$.pipe(
                ofType<SelectFilterTableColumnsMSA>(
                    ConfigurationActionTypes.SelectFilterTableColumnsMSA,
                ),
                mergeMap((action) => {
                    const tableType = action.payload.type;
                    const oldColumnOrder = action.payload.columnOrder;
                    const mapColumnToOption: (c: TableColumn) => Option = (
                        column,
                    ) => ({
                        value: column.id,
                        ...
                        tooltip: column.dataIsUnavailable
                            ? "Data is not available."
                            : "",
                    });
                    const dialogData: DialogSelectData = {
                        title:
                            tableType === TableType.STATIONS
                                ? "Station Columns"
                                : "Delivery Columns",
                        sorting: action.payload.columnOrder,
                        favouriteOptions:
                            action.payload.favouriteColumns.map(
                                mapColumnToOption,
                            ),
                        otherOptions:
                            action.payload.otherColumns.map(mapColumnToOption),
                    };
                    ...
                    this.dialogService
                        ...
                        .subscribe(
                            (result: DialogResultData) => {
                                if (result && result.sorting) {
                                    if (tableType === TableType.STATIONS) {
                                        this.store.dispatch(
                                            new SetFilterStationTableColumnOrderSOA(
                                                { columnOrder: result.sorting },
                                            ),
                                        );
                                    } else if (
                                        tableType === TableType.DELIVERIES
                                    ) {
                                        this.store.dispatch(
                                            new SetFilterDeliveryTableColumnOrderSOA(
                                                { columnOrder: result.sorting },
                                            ),
                                        );
                                    }
                            ...
                            (error) => {
                                throw new Error(
                                    `error loading dialog or selecting columns: ${error}`,
                                );
                            },
                        );
                    return EMPTY;
    ...
    DeleteHighlightingRuleSSA$ = createEffect(
        () =>
            this.actions$.pipe(
                ofType<DeleteHighlightingRuleSSA>(
                    ConfigurationActionTypes.DeleteHighlightingRuleSSA,
                ),
                withLatestFrom(
                    this.store.pipe(select(selectHighlightingSettings)),
                ),
                mergeMap(([action, state]) => {
                    const deleteRuleId =
                        action.payload.deleteRequestData.ruleId;
                    const stationRuleToDelete =
                        state.stations.find(
                    ...
                    const ruleToDelete =
                        stationRuleToDelete || deliveryRuleToDelete;

                    if (ruleToDelete !== null) {
                        const xPos = (
                            action.payload.deleteRequestData.xPos - 350
                        )
                            .toString(10)
                            .concat("px");
                        const yPos = (
                            action.payload.deleteRequestData.yPos - 140
                        )
                            .toString(10)
                            .concat("px");
                        ...
                        this.dialogService
                            .open(DialogYesNoComponent, dialogConfig)
                            .afterClosed()
                            .pipe(take(1))
                            .subscribe(
                                (result) => {
                                    if (result === true) {
                                        if (stationRuleToDelete !== null) {
                                            const newRules =
                                                this.editHighlightingService.removeRule(
                                                    state.stations,
                                                    deleteRuleId,
                                                );
                                            this.store.dispatch(
                                                new SetStationHighlightingRulesSOA(
                                                    { rules: newRules },
                                                ),
                                            );
                                        } else {
                                            ...
                                            this.store.dispatch(
                                                new SetDeliveryHighlightingRulesSOA(
                                                    { rules: newRules },
                                                ),
                                            );
                                        }
                                ...
                                (error) => {
                                    throw new Error(
                                        `error loading YesNo dialog: ${error}`,
                                    );
                                },
                            );
                    }
