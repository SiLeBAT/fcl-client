import { Component, Input, Output, EventEmitter, ChangeDetectionStrategy, ElementRef, ViewChild, ChangeDetectorRef } from '@angular/core';
...
export class ColorSelectorViewComponent {
    ...
    @ViewChild(ColorPickerDirective, { static: false }) colorPickerDirective: ColorPickerDirective;
    ...
    get ngClass(): string {
        return this.spaceIsNotAvailable ? ColorSelectorViewComponent.WO_SPACE_CLASS : '';
    }
    ...
    private getPreferredPositions(): ColorPickerPosition[] {
        return [
            ...
            ...this.defaultColorPickerPositions.filter(p => p !== this.preferredColorPickerPosition)
        ];
    }
    ...
    private updateColorPickerPosition(): void {
        ...
        for (const pos of positions) {
            if (
                (pos === 'left' && space.left >= ColorSelectorViewComponent.COLOR_PICKER_WIDTH) ||
                (pos === 'top' && space.top >= ColorSelectorViewComponent.COLOR_PICKER_HEIGHT) ||
                (pos === 'right' && space.right >= ColorSelectorViewComponent.COLOR_PICKER_WIDTH) ||
                (pos === 'bottom' && space.bottom >= ColorSelectorViewComponent.COLOR_PICKER_HEIGHT)) {
