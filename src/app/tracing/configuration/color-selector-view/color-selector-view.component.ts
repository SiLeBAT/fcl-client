...
export class ColorSelectorViewComponent {
  ...
  get style(): any {
    return this.rgbColorStr_ ? {'background-color': this.rgbColorStr_} : {};
  }
  ...
  private updateColorPickerPosition(): void {
    ...
    for (const pos of positions) {
      if (
        (pos === 'left' &&
          space.left >= ColorSelectorViewComponent.COLOR_PICKER_WIDTH) ||
        (pos === 'top' &&
          space.top >= ColorSelectorViewComponent.COLOR_PICKER_HEIGHT) ||
        (pos === 'right' &&
          space.right >= ColorSelectorViewComponent.COLOR_PICKER_WIDTH) ||
        (pos === 'bottom' &&
          space.bottom >= ColorSelectorViewComponent.COLOR_PICKER_HEIGHT)
      ) {
