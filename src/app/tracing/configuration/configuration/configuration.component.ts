...
export class ConfigurationComponent implements OnInit {
    ...
    @ViewChild("highlightingTemplate", { static: true })
    highlightingTemplate: TemplateRef<any>;
    @ViewChild("settingsTemplate", { static: true })
    settingsTemplate: TemplateRef<any>;
    tabConfigs: AddTypedId<TabConfig, ActiveConfigurationTabId>[] = [];

    activeTabIndex$ = this.store
        .select(tracingSelectors.getActiveConfigurationTabId)
        .pipe(
            map(
                (tabId) =>
                    this.tabConfigs.findIndex(
                        (tabConfig) => tabConfig.id === tabId,
                    ) || 0,
            ),
        );
    ...
    onTabChange(tabIndex: number) {
        ...
        this.store.dispatch(
            new tracingActions.SetActiveConfigurationTabIdSOA({
                activeConfigurationTabId: tabId,
            }),
        );
    }
