...
export class HighlightingDeliveryComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        const isHighlightingDeliveryTabActive$ = this.store.select(tracingSelectors.selectIsHighlightingDeliveryTabActive);
        const deliveryHighlightingState$ = this.store.select(tracingSelectors.selectDeliveryHighlightingState);
        this.stateSubscription = deliveryHighlightingState$.pipe(optInGate(isHighlightingDeliveryTabActive$, true)).subscribe(
            (state) => this.applyState(state),
            err => this.alertService.error(`getDeliveryHighlightingData store subscription failed: ${err}`)
        );
    }
    ...
    onRuleOrderChange(ruleIds: RuleId[]): void {
        if (this.cachedState) {
            const newRules = this.editHighlightingService.applyRuleOrderChange(
                ruleIds, this.cachedState.dataServiceInputState.highlightingSettings.deliveries
            );
    ...
    onToggleRuleIsDisabled(ruleId: RuleId): void {
        if (this.cachedState) {
            const newRules = this.editHighlightingService.toggleRuleIsDisabled(
                ruleId, this.cachedState.dataServiceInputState.highlightingSettings.deliveries
            );
            this.emitNewRules(newRules);
        }
    }
    ...
    onStartEdit(ruleId: RuleId): void {
        if (this.cachedState) {
            const newEditRule = this.editHighlightingService.createEditRuleFromDeliveryRule(
                ruleId, this.cachedState.dataServiceInputState.highlightingSettings.deliveries
            );
            if (newEditRule) {
    ...
    onAddSelectionToRuleConditions(editRule: DeliveryEditRule): void {
        if (this.cachedState) {
            const updatedEditRule = this.editHighlightingService.addSelectionToDeliveryRuleConditions(
    ...
    private emitNewEditRules(editRules: DeliveryEditRule[]): void {
        this.store.dispatch(new tracingActions.SetDeliveryHighlightingEditRulesSOA({ editRules: editRules }));
    }

    private emitNewRules(rules: DeliveryHighlightingRule[]) {
        this.store.dispatch(new tracingActions.SetDeliveryHighlightingRulesSOA({ rules: rules }));
    }

    private saveRule(editRule: DeliveryEditRule): void {
        const newRules = this.editHighlightingService.applyDeliveryRule(
            editRule, this.cachedState!.dataServiceInputState.highlightingSettings.deliveries
        );
        this.emitNewRules(newRules);
    ...
    private applyState(state: EditHighlightingState<DeliveryEditRule>): void {
        this.cachedData = this.editHighlightingService.getDataForEditDeliveryHighlightingRules(state.dataServiceInputState);
