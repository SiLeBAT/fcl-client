...
import {
    convertDeliveryHRuleToEditRule,
    convertDeliveryHRuleToRuleListItem,
    convertStationHRuleToRuleListItem,
    convertStationHRuleToEditRule,
    convertDeliveryEditRuleToHRule,
    convertStationEditRuleToHRule,
...
const PREFERRED_DELIVERY_THICKNESS_PROP_IDS = [
    "score",
    "amountNumber",
    "lotAmountNumber",
];

const EMPTY_STATION_THICKNESS_PROPERTIES: PropertySets = {
    favouriteProperties: [],
    otherProperties: [],
};
...
export class EditHighlightingService {
    ...
    private static readonly hiddenDeliveryProps =
        EditHighlightingService.hiddenProps;
    private static readonly hiddenStationProps =
        EditHighlightingService.hiddenProps;
    ...
    getClearInvisiblitiesPayload(
        ...
        if (options.stations || options.deliveries) {
            return {
                highlightingSettings: {
                    ...state,
                    invisibleStations: options.stations
                        ? []
                        : state.invisibleStations,
                    invisibleDeliveries: options.deliveries
                        ? []
                        : state.invisibleDeliveries,
                },
            };
        }
        return null;
    ...
    applyRuleOrderChange<T extends HighlightingRule>(
        ruleIds: RuleId[],
        rules: T[],
    ): T[] {
        const newRules = rules.slice();
        let oldIndices = ruleIds.map((id) =>
    ...
    toggleRuleIsDisabled<T extends HighlightingRule>(
        ruleId: RuleId,
        rules: T[],
    ): T[] {
        ...
        if (oldRule !== null) {
            rules = this.updateRule(
                oldRule,
                { userDisabled: !oldRule.userDisabled },
                rules,
            );
        }
        return rules;
    ...
    toggleShowRuleInLegend<T extends HighlightingRule>(
        ruleId: RuleId,
        rules: T[],
    ): T[] {
    ...
    private updateRule<T extends HighlightingRule>(
        rule: T,
        update: Partial<HighlightingRule>,
        rules: T[],
    ): T[] {
        if (rule !== null) {
            const newRule: T = {
            ...
            if (isEnabledAdjustThicknessRule(newRule)) {
                rules = getUpdatedArray(
                    rules,
                    (r) =>
                        r.id !== newRule.id && isEnabledAdjustThicknessRule(r),
                    { userDisabled: true } as Partial<T>,
                );
            }
            rules = this.applyRule(newRule, rules);
            rules = updateDisabledFlags(rules);
        }
        return rules;
    }

    addSelectionToStationRuleConditions<T extends StationEditRule>(
        editRule: T,
    ...
    addSelectionToDeliveryRuleConditions<T extends DeliveryEditRule>(
        editRule: T,
        state: DataServiceInputState,
    ): T {
    ...
    private addSelectionToRuleConditions<T extends EditRule>(
        editRule: T,
        selectedIds: string[],
    ): T {
        const listedIds = this.getListedIdsFromConditions(
        ...
        if (addIds.length > 0) {
            ...
            const index = this.getLastNonEmptyConditionIndex(
                editRule.complexFilterConditions,
            );
    ...
    removeSelectionFromStationRuleConditions<T extends StationEditRule>(
        editRule: T,
        state: DataServiceInputState,
    ): T {
        const selectedIds = this.getSelectedStationIdsFromState(state);
        return this.removeSelectionFromEditRuleConditions(
            editRule,
            selectedIds,
        );
    }

    removeSelectionFromDeliveryRuleConditions<T extends EditRuleCore>(
        editRule: T,
        state: DataServiceInputState,
    ): T {
        const selectedIds = this.getSelectedDeliveryIdsFromState(state);
    ...
    private removeSelectionFromEditRuleConditions<T extends EditRuleCore>(
        editRule: T,
        selectedIds: string[],
    ): T {
        const newConditions = this.filterConditionsForIds(
            editRule.complexFilterConditions,
            selectedIds,
        );
    ...
    private getListedIdsFromConditions(
    ...
    private getSelectedStationIdsFromState(
        state: DataServiceInputState,
    ): StationId[] {
        const stations = this.dataService.getData(state).stations;
        return stations
            .filter((s) => !s.invisible && !s.contained && s.selected)
            .map((s) => s.id);
    }

    private getSelectedDeliveryIdsFromState(
        state: DataServiceInputState,
    ): DeliveryId[] {
        const deliveries = this.dataService.getData(state).deliveries;
        return deliveries
            .filter((d) => !d.invisible && d.selected)
            .map((d) => d.id);
    }

    private getLastNonEmptyConditionIndex(
        conditions: ComplexFilterCondition[],
    ): number {
    ...
    private filterConditionsForIds(
        conditions: ComplexFilterCondition[],
        ids: string[],
    ): ComplexFilterCondition[] {
    ...
    createEditRuleFromDeliveryRule(
        ruleId: RuleId,
        rules: DeliveryHighlightingRule[],
    ): DeliveryEditRule | null {
        const rule = this.getRuleFromId(ruleId, rules);
        return rule === null ? null : convertDeliveryHRuleToEditRule(rule);
    }

    private getRuleFromId<T extends HighlightingRule | EditRule>(
        ruleId: RuleId,
        rules: T[],
    ): T | null {
    ...
    removeRule<T extends EditRule | HighlightingRule>(
        rules: T[],
        ruleId: RuleId,
    ): T[] {
    ...
    private updateCache(state: DataServiceInputState): void {
        ...
        let deliveryRuleListItems = cacheIsEmpty
            ? null
            : this.cachedData!.deliverySpecificData.ruleListItems;
        let stationRuleListItems = cacheIsEmpty
            ? null
            : this.cachedData!.stationSpecificData.ruleListItems;
        let deliveryPropData = cacheIsEmpty
            ? null
            : this.cachedData!.deliverySpecificData.propData;
        let stationPropData = cacheIsEmpty
            ? null
            : this.cachedData!.stationSpecificData.propData;

        if (
            cacheIsEmpty ||
            this.cachedData!.highlightingStats !==
                dataServiceData.highlightingStats
        ) {
            // update rule lists
    ...
    private getPropertiesForThickness(
        dataTable: DataTable,
        rules: HighlightingRule[],
        favouritePropIds: string[],
    ): PropertySets {
        const usedPropIds = new Set(
            removeUndefined(
                rules
                    .filter((r) => r.adjustThickness)
                    .map((r) => r.valueCondition?.propertyName),
            ),
        );
        const favPropIds = new Set(favouritePropIds);
        const favColumns = dataTable.columns.filter((c) =>
            favPropIds.has(c.id),
        );
        const otherColumns = dataTable.columns.filter(
            (c) =>
                !favPropIds.has(c.id) &&
                (c.type === "number" || usedPropIds.has(c.id)),
        );
        return {
            favouriteProperties: [],
            otherProperties: [...favColumns, ...otherColumns],
        };
    }
    ...
    private createDeliveryPropData(state: DataServiceInputState): PropData {
        ...
        return {
            ...
            propertiesForThickness: this.getPropertiesForThickness(
                dataTable,
                state.highlightingSettings.deliveries,
                PREFERRED_DELIVERY_THICKNESS_PROP_IDS,
            ),
            propToValuesMap: extractPropToValuesMap(
                dataTable.rows,
                dataTable.columns,
            ),
        };
    ...
    private createStationPropData(state: DataServiceInputState): PropData {
        ...
        return {
            favouriteProperties: dataTable.favouriteColumns,
            otherProperties: dataTable.otherColumns,
            propertiesForThickness: EMPTY_STATION_THICKNESS_PROPERTIES,
            propToValuesMap: extractPropToValuesMap(
                dataTable.rows,
                dataTable.columns,
            ),
