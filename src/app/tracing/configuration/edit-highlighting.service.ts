...
export class EditHighlightingService {
  ...
  private updateAutoDisabledFlag<T extends HighlightingRule>(rules: T[]): T[] {
    ...
    indicesOfRulesToUpdate = rules.reduce(
      (prevResult, rule, index) =>
        isUpdateRequiredCheckFun(rule) ? [...prevResult, index] : prevResult,
      [] as number[]
    );
    if (indicesOfRulesToUpdate.length > 0) {
      rules = rules.slice();
      indicesOfRulesToUpdate.forEach(
        i => (rules[i] = {...rules[i], autoDisabled: hasActiveAnoRule})
      );
    }
    return rules;
  }
  ...
  private updateCache(state: DataServiceInputState): void {
    ...
    if (
      cacheIsEmpty ||
      this.cachedData!.highlightingStats !== dataServiceData.highlightingStats
    ) {
      // update rule lists
      deliveryRuleListItems = state.highlightingSettings.deliveries.map(rule =>
        convertDeliveryHRuleToRuleListItem(
          rule,
          dataServiceData.highlightingStats!
        )
      );
      stationRuleListItems = state.highlightingSettings.stations.map(rule =>
        convertStationHRuleToRuleListItem(
          rule,
          dataServiceData.highlightingStats!
        )
