...
export class MergeStationsDialogComponent {
    inputFormControl = new FormControl("", (control: AbstractControl) =>
        this.isInputValid(control),
    );
    ...
    constructor(
        ...
        this.containedGroups = data.state.groupSettings.filter(
            (g) => memberSet[g.id],
        );
    ...
    onOk() {
        const payload = this.groupingService.getMergeStationsPayload(
            this.data.state,
    ...
    private isInputValid(control: AbstractControl): ValidationErrors | null {
        if (control.dirty) {
            const validationCode =
                this.groupingService.validateMergeStationsCmd(
                    this.data.state,
                    control.value,
                    this.data.memberIds,
                );
