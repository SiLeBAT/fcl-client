...
export class GroupingEffects {
    ...
    collapseStations$ = createEffect(() => this.actions$.pipe(
        ...
        mergeMap(([action, state]) => {
            ...
            try {
                const payload = this.groupingService.getCollapseStationsPayload(state, groupType, groupMode);
                ...
                this.alertService.error(`Stations could not be collapsed!, error: ${error}`);
                return EMPTY;
            }
        })
    ));
    ...
    uncollapseStations$ = createEffect(() => this.actions$.pipe(
        ...
        mergeMap(([action, state]) => {
            ...
            try {
                ...
                this.alertService.error(`Stations could not be uncollapsed!, error: ${error}`);
                return EMPTY;
            }
        })
    ));
