  ...
  collapseStations$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(([action, state]) => {
        ...
        try {
          const payload = this.groupingService.getCollapseStationsPayload(
            state,
            groupType,
            groupMode
          );
  ...
  uncollapseStations$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(([action, state]) => {
        const groupType = action.payload.groupType;
        try {
          const payload = this.groupingService.getUncollapseStationsPayload(
            state,
            groupType
          );
  ...
  expandStations$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(([action, state]) => {
        ...
        try {
          const payload = this.groupingService.getExpandStationsPayload(
            state,
            stationIds
          );
