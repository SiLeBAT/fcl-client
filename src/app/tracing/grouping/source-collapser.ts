...
export class SourceCollapser {
  ...
  getGroupingChange(
    ...
    for (const source of sourceStationCandidates) {
      if (
        data.getDelById(source.incoming).filter(d => !d.invisible).length === 0
      ) {
        ...
        const targetStations: StationData[] = data
          .getStatById(_.uniq(sourceOutDeliveries.map(d => d.originalTarget)))
            ...
            const targetOutDeliveries: DeliveryData[] = data
              .getDelById(
                targetStations[0].connections
                  .filter(c => targetInDeliveryIdSet.has(c.source))
                  .map(c => c.target)
              )
              .filter(d => !d.invisible);
