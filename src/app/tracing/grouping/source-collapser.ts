    ...
    getGroupingChange(state: GroupingState, groupMode: GroupMode): GroupingChange {
        ...
        const sourceStationCandidates: StationData[] = data.stations.filter(
            s =>
                ...
                (s.outgoing && s.outgoing.length > 0) &&
        ...
        for (const source of sourceStationCandidates) {
            ...
            if (data.getDelById(source.incoming).filter(d => !d.invisible).length === 0) {

                const sourceOutDeliveries: DeliveryData[] = data.getDelById(source.outgoing).filter(d => !d.invisible);
                const targetStations: StationData[] = data.getStatById(
                    _.uniq(sourceOutDeliveries.map(d => d.originalTarget))
                ...
                if (targetStations.length === 1) {
                    ...
                    if (groupMode === GroupMode.WEIGHT_ONLY) {
                        ...
                        const targetInDeliveryIdSet: Set<string> = new Set(
                            sourceOutDeliveries.filter(d => d.originalTarget === targetStations[0].id).map(d => d.id)
                        );
                        const targetOutDeliveries: DeliveryData[] = data.getDelById(
                            targetStations[0].connections.filter(c => targetInDeliveryIdSet.has(c.source)).map(c => c.target)
                        ...
                        if (targetOutDeliveries.length === 0) {
                            ...
                            switch (groupMode) {
                                case GroupMode.PRODUCT_AND_WEIGHT:
                                    targetKeys = _.uniq(targetOutDeliveries.map(
                                        d => 'W>0:' + (source.outbreak ? '1' : '0') + '_P:' + d.name || d.id
                                    )).sort();
                                    break;
                                case GroupMode.LOT_AND_WEIGHT:
                                    targetKeys = _.uniq(targetOutDeliveries.map(
                                        d => 'W>0:' + (source.outbreak ? '1' : '0') + '_L:' + d.lotKey
                                    )).sort();
                                    break;
                                default:
                                    // unkown mode
