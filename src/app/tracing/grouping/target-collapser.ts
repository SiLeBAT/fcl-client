...
export class TargetCollapser {
    ...
    getGroupingChange(
        ...
        const targetStationCandidates: StationData[] = data.stations.filter(s =>
            ...
            (s.incoming && s.incoming.length > 0) &&
        ...
        for (const target of targetStationCandidates) {
            ...
            if (data.getDelById(target.outgoing).filter(d => !d.invisible).length === 0) {

                const targetInDeliveries: DeliveryData[] = data.getDelById(target.incoming).filter(d => !d.invisible);
                const sourceStations: StationData[] = data.getStatById(
                ...
                if (sourceStations.length === 1) {
                    ...
                    if (groupMode === GroupMode.WEIGHT_ONLY) {
                        ...
                        const sourceOutDeliveries = targetInDeliveries.filter(d => d.originalSource === sourceStations[0].id);
                        if (sourceOutDeliveries.length === 0) {
                            sourceKeys = ['W>0:' + (target.outbreak ? '1' : '0')];
                        } else {
                            switch (groupMode) {
                                case GroupMode.PRODUCT_AND_WEIGHT:
                                    sourceKeys = _.uniq(sourceOutDeliveries.map(d =>
                                        'W>0:' + (target.outbreak ? '1' : '0') + '_P:' + (d.name ? 'PN:' + d.name : 'DID:' + d.id)
                                    )).sort();
                                    break;
                                case GroupMode.LOT_AND_WEIGHT:
                                    sourceKeys = _.uniq(sourceOutDeliveries.map(d =>
                                        'W>0:' + (target.outbreak ? '1' : '0') + '_L:' +
                                        (d.name && d.lot ? 'PN:' + d.name + 'LN:' + d.lot : 'DID:' + d.id)
                                    )).sort();
                                    break;
                                default:
                                    // unkown mode
