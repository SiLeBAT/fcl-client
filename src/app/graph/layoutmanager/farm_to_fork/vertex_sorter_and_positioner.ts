...
function sortVertices(graph: Graph) {
  ...
  const splitCount: number = _.sum(graph.layers.map(layer=>_.sum(layer.map(vertex=>((vertex.inEdges.length>=2?1:0)+(vertex.outEdges.length>=2?1:0))))));
  //console.log('maxLayerSize:' + maxLayerSize.toString() + ', graphSize: ' + graphSize.toString() + ', splitCount: ' + splitCount.toString());
  //if(maxLayerSize>6 || graphSize>30 || splitCount>10) sortVerticesWithHeuristic(graph);
  if(maxLayerSize>8 || graphSize>30 || splitCount>12) sortVerticesWithHeuristic(graph);
