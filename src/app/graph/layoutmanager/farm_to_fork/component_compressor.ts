...
/*export function compressSimpleConnectedComponents(graph: Graph) {
...
export function compressSimpleTargets(graph: Graph, vertexDistance: number) {
  for(const layer of graph.layers) {
    ...
    // fill source -> targets[] map
    for(const target of layer.filter(v=>v.outEdges.length==0 && v.inEdges.length==1)) {
      if(!sourceToTargetMap.has(target.inEdges[0].source.index)) sourceToTargetMap.set(target.inEdges[0].source.index, [target]);
