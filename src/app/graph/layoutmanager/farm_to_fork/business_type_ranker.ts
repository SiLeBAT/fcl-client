...
export class BusinessTypeRanker {
  ...
  constructor(private sinkTypes: String[], private sourceTypes: String[], typeOrderings: String[][]) {
    this.init(sinkTypes, sourceTypes, typeOrderings);
  }

  private init(sinkTypes: String[], sourceTypes: String[], typeOrderings: String[][]) {
    // register businessTypes
    for(let s of sinkTypes) this.isSink[this.add(s)] = true;
    for(let s of sourceTypes) this.isSource[this.add(s)] = true;
    for(let orderedChain of typeOrderings) for(let s of orderedChain) this.add(s);

    let typeCount: number = this.indexMap.size;
    ...
    for(let i: number = typeCount-1; i>=0; i--) this.rankMatrix[typeCount-1][i] = null;
    for(let i: number = typeCount-2; i>=0; i--) this.rankMatrix[i] =this.rankMatrix[typeCount-1].slice();

    let sinkCodes = sinkTypes.map(t => {return this.indexMap.get(t)});
    let sourceCodes = sourceTypes.map(t => {return this.indexMap.get(t)});

    for(let sinkCode of sinkCodes) for(let i: number = typeCount-1; i>=0; i--) if(!this.isSink[i]) this.rankMatrix[sinkCode][i] = true;
   
    for(let orderedChain of typeOrderings) {
      for(let i: number = orderedChain.length-2; i>0; i--) {
        this.rankMatrix[this.indexMap.get(orderedChain[i])][this.indexMap.get(orderedChain[i+1])] = true;
      }
    }
    // make rank matrix transitive
    for(let i: number = 0; i<typeCount; i++ ) for(let j: number = 0; j<typeCount; j++) {
      if(i!==j) for(let k: number = 0; k<typeCount; k++) if(this.rankMatrix[j][i] && this.rankMatrix[i][k]) this.rankMatrix[j][k] = true;
    }
  }
  ...
  private add(type: String): number {
    ...
    let index: number = this.indexMap.size;
