...
export class GraphEditorComponent implements AfterViewInit, OnDestroy {
    ...
    constructor(
        ...
        this.store.dispatch(
            new editorActions.GraphEditorActivated({ isActivated: true }),
        );
    }
        ...
        mxUtils.getAll(
            ...
            (xhr: any) => {
                ...
                this.editorUi = new EditorUi(
                    editor,
                    that.editorContainer.nativeElement,
                );
                if (graphModel) {
    ...
    ngOnDestroy() {
        this.store.dispatch(
            new editorActions.GraphEditorActivated({ isActivated: false }),
        );
    }

    canDeactivate(): Observable<boolean> {
        const diagramIsEmpty = this.editorUi
            ? this.editorUi.isDiagramEmpty()
            : true;
        if (!diagramIsEmpty) {
