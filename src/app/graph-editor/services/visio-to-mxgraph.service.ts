...
import * as jsonData from '../../../assets/data/f2f.visio.json';
...
export class VisioToMxGraphService {
    private data: VisioReport = jsonData as any;
    ...
    createGraph(report: VisioReport) {
        ...
        try {
            ...
            _.forIn(rootElements, (stationGroup: VisioBox) => {
                const stationGroupCell: mxCell = this.drawCell(parent, stationGroup);
                this.pushToBoxes(stationGroupCell, stationGroup);
                if (stationGroup.label) {
                    const stationGroupLabel: mxCell = this.drawLabel(stationGroupCell, stationGroup.label);
                }

                // draw station elements
                const stationElements: VisioBox[] = stationGroup.elements;
                _.forIn(stationElements, (station: VisioBox) => {
                    const stationCell: mxCell = this.drawCell(stationGroupCell, station);
                    this.pushToBoxes(stationCell, station);
                    if (station.label) {
                        const stationLabel: mxCell = this.drawLabel(stationCell, station.label);
                    }

                    // draw lot elements
                    const lotElements: VisioBox[] = station.elements;
                    _.forIn(lotElements, (lot: VisioBox) => {
                        const lotCell: mxCell = this.drawCell(stationCell, lot);
                        this.pushToBoxes(lotCell, lot);
                        if (lot.label) {
                            const lotLabel: mxCell = this.drawLabel(lotCell, lot.label);
                        }
                    });
                });
