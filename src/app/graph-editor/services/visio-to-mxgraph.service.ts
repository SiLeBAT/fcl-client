...
function connectorStyle(exitX = 0.5, exitY = 1, entryX = 0.5, entryY = 0) {
  return getStyle(
...
export class VisioToMxGraphService {
  ...
  createGraph(report: VisioReport) {
    ...
    this.boxes.forEach(box =>
      box.visioBox.ports.forEach(
        port => (id2BoxPort[port.id] = {box: box, currentPort: port})
      )
    );
  ...
  private drawLegend(
    ...
    const nEntries =
      Object.keys(legendEntries).filter(key => legendEntries[key]).length +
      (showDeliveries ? 1 : 0);
    if (!nEntries) {
      return null;
    }
    ...
    legendEntryMatrix.forEach(legendColumnEntries => {
      const columnCells = legendColumnEntries.map((legendEntry, rowIndex) =>
        this.drawLegendEntry(legendCell, legendEntry, {
          ...
          y:
            startY +
            rowIndex * (LEGEND_ENTRY_SIZE.height + LEGEND_ENTRY_DIST.y),
        })
      );
      xPos +=
